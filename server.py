...
_VAPID_DIR = os.path.dirname(os.path.abspath(__file__))
_VAPID_PRIVATE_KEY = os.path.join(_VAPID_DIR, ".vapid_private.pem")
_VAPID_PUBLIC_KEY_FILE = os.path.join(_VAPID_DIR, ".vapid_keys.json")
_PUSH_SUBS_FILE = os.path.join(_VAPID_DIR, ".push_subscriptions.json")
_push_lock = threading.Lock()
_push_stop = threading.Event()


def _load_vapid_public_key():
    try:
        with open(_VAPID_PUBLIC_KEY_FILE, "r") as f:
...
def _save_push_subscriptions(subs):
    dir_name = os.path.dirname(_PUSH_SUBS_FILE)
    fd, tmp_path = tempfile.mkstemp(dir=dir_name, suffix=".tmp")
    try:
        with os.fdopen(fd, "w") as f:
            json.dump(subs, f)
        os.replace(tmp_path, _PUSH_SUBS_FILE)
    except Exception:
        try:
            os.unlink(tmp_path)
        except OSError:
...
def get_vapid_key():
    return jsonify({"key": _load_vapid_public_key()})


@app.route("/api/push/subscribe", methods=["POST"])
def push_subscribe():
    body = request.get_json(silent=True)
    if not body or "endpoint" not in body or "keys" not in body:
        return jsonify({"error": "invalid subscription"}), 400
    endpoint = str(body["endpoint"])[:2000]
    keys = body.get("keys", {})
    ...
    sub = {
        "endpoint": endpoint,
        "keys": {
            "p256dh": str(keys["p256dh"])[:200],
            "auth": str(keys["auth"])[:100],
        }
    }
    with _push_lock:
...
def push_unsubscribe():
    body = request.get_json(silent=True)
    endpoint = str(body.get("endpoint", ""))[:2000] if body else ""
    with _push_lock:
        subs = _load_push_subscriptions()
        subs = [s for s in subs if s.get("endpoint") != endpoint]
        _save_push_subscriptions(subs)
...
def _send_push_notifications():
    ...
    if not os.path.exists(_VAPID_PRIVATE_KEY):
        return
    ...
    payload = json.dumps({
        "title": "Nudge — DO IT!!!",
        "body": f"You have {len(data['reminders'])} reminder{'s' if len(data['reminders']) != 1 else ''} waiting.",
        "tag": "nudge-reminder",
    })
    ...
    for sub in subs:
        try:
            webpush(
                ...
                vapid_claims={"sub": "mailto:nudge@localhost"},
            )
        except Exception:
            # Subscription likely expired or invalid
...
def _push_timer_loop(stop_event):
    """Background thread that sends push notifications at the configured interval."""
    while not stop_event.is_set():
        data = _load_data()
        interval_min = data.get("config", {}).get("popup_interval_minutes", 60)
        interval_min = max(1, int(interval_min)) if isinstance(interval_min, (int, float)) else 60
...
def start_push_timer():
    """Start the push notification timer thread."""
    t = threading.Thread(target=_push_timer_loop, args=(_push_stop,), daemon=True)
    t.start()
    return t


# Auto-start push timer when server module loads
start_push_timer()
